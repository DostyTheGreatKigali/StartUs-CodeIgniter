...
class Training extends REST_Controller
{
    ...
    public function index()
    {
        ...
        @$cat_id = $this->app_model->catidBySlug('home');
        ...
        @$data['client']           = $this->app_model->article($this->app_model->catidBySlug('client')->cat_id);
        ...
        @$data['testimonial']    = $this->app_model->article($this->app_model->catidBySlug('testimonial')->cat_id);
        @$data['about']          = $this->app_model->article($this->app_model->catidBySlug('about')->cat_id);
        $data['article']        = $this->app_model->article($cat_id->cat_id);
        $data['slider']         = $this->app_model->active_slider();
    ...
    public function page()
    {
        ...
        if (!$this->app_model->catidBySlug($this->uri->segment(1))) {
            redirect(base_url());
        }
        ...
        $data['article']    = $this->app_model->article($this->app_model->catidBySlug($this->uri->segment(1))->cat_id);
    ...
    public function coinmarket()
    {
        ...
        $data['title']          = $this->uri->segment(1);
        $data['advertisement']  = $this->app_model->advertisement($cat_id->cat_id);
        $data['article']        = $this->app_model->article($cat_id->cat_id);
        $data['cat_info']       = $this->app_model->cat_info($this->uri->segment(1));
        ...
        $data['cryptocoins']    = $this->app_model->cryptoCoin($config["per_page"], $page);
    ...
    public function price()
    {
        ...
        $data['title']          = $this->uri->segment(1);
        $data['advertisement']  = $this->app_model->advertisement($cat_id->cat_id);
        @$data['newscat']        = $this->app_model->newsCatListBySlug('news');
        $data['article']        = $this->app_model->article($cat_id->cat_id);
        $data['cat_info']       = $this->app_model->cat_info($this->uri->segment(1));
    ...
    public function about()
    {
        ...
        $data['title']          = $this->uri->segment(1);
        @$data['client']           = $this->app_model->article($this->app_model->catidBySlug('client')->cat_id);
        @$data['team']           = $this->app_model->article($this->app_model->catidBySlug('team')->cat_id);
        @$data['testimonial']    = $this->app_model->article($this->app_model->catidBySlug('testimonial')->cat_id);
        $data['article']        = $this->app_model->article($cat_id->cat_id);
        $data['cat_info']       = $this->app_model->cat_info($this->uri->segment(1));
    ...
    public function service($slug = NULL)
    {
        ...
        if ($slug == "" || $slug == NULL) {
            ...
            $data['cat_info']           = $this->app_model->cat_info($this->uri->segment(1));
            $data['service_details']    = $this->app_model->contentDetails($this->uri->segment(2));
    ...
    public function news_get()
    {
        ...
        //For Coin Tricker
        $data['cryptocoins']        = $this->app_model->cryptoCoin(10, 0);
        $data['recentnews']         = $this->db->select("*")->from('web_news')->order_by('article_id', 'desc')->limit(3)->get()->result();

        if ($slug2 == "" || $slug2 == NULL || is_numeric($slug2)) {

            //All Category News with Pagination
            $cat_id     = $this->app_model->catidBySlug($slug1)->cat_id;
            $where_add  = $this->app_model->catidBySlug('news')->cat_id;
            ...
            $data['advertisement']  = $this->app_model->advertisement($where_add);
            $data['newscat']        = $this->app_model->newsCatListBySlug('news');
            $data['cat_info']       = $this->app_model->cat_info($slug1);
            $data['content']        = $this->load->view("website/sidebar", $data, true);

            return $this->response(['success' => TRUE, 'message' => 'News item 1', 'item1' => $data], REST_Controller::HTTP_OK);
            ...
            //Slug Category News
            $cat_id     = $this->app_model->catidBySlug($slug2)->cat_id;
            /******************************
             * Pagination Start
             ******************************/
            ...
            return $this->response(['success' => TRUE, 'message' => 'News item 2', 'item2' => $data], REST_Controller::HTTP_OK);
            ...
            return $this->response(['success' => TRUE, 'message' => 'News item 3', 'item3' => $data], REST_Controller::HTTP_OK);
            ...
            $where_add = $this->app_model->catidBySlug('news-details')->cat_id;
            $data['advertisement']  = $this->app_model->advertisement($where_add);

            @$data['newscat']        = $this->app_model->newsCatListBySlug('news');
            $data['article']        = $this->app_model->article($slug1);
            $data['cat_info']       = $this->app_model->cat_info($slug1);
            $data['news']           = $this->app_model->newsDetails($slug3);
            // $data['content']        = $this->load->view("website/sidebar", $data, true);

            return $this->response(['success' => TRUE, 'message' => 'News item 4', 'item4' => $data], REST_Controller::HTTP_OK);
    ...
    public function faq_get()
    {
        ...
        return $this->response(['success' => TRUE, 'message' => 'News item 4', 'faq' => $data], REST_Controller::HTTP_OK);
        ...
        if ($this->form_validation->run() === true) {
            ...
            if ($usercheck->num_rows() > 0) {
                if ($usercheck->row()->oauth_provider == 'facebook' && $usercheck->row()->status == 0 || $usercheck->row()->oauth_provider == 'google' && $usercheck->row()->status == 0) {
                    ...
                    $this->app_model->updateUser($data);
    ...
    public function login()
    {
        ...
        @$cat_id = $this->app_model->catidBySlug('register');
        ...
        //From Validation Check
        if ($this->form_validation->run()) {
            $user = $this->app_model->checkUser($userData);
            if ($user->num_rows() > 0) {
    ...
    public function subscribe()
    {
        ...
        if ($this->app_model->subscribe($data)) {
    ...
    public function activeAcc($activecode = '')
    {
        ...
        $user = $this->app_model->activeAccountSelect($activecode);

        if ($user->num_rows() > 0) {
            $this->app_model->activeAccount($activecode);
    ...
    public function paymentform()
    {
        ...
        @$cat_id = $this->app_model->catidBySlug('buy');
        ...
        $data['title']      = $this->uri->segment(1);
        $data['article']    = $this->app_model->article($cat_id->cat_id);
        $data['cat_info']   = $this->app_model->cat_info('buy');
    ...
    public function resetPassword()
    {
        ...
        @$cat_id = $this->app_model->catidBySlug('forgot-password');

        $data['title'] = $this->uri->segment(1);
        $data['article']  = $this->app_model->article($cat_id->cat_id);
        $data['cat_info'] = $this->app_model->cat_info('forgot-password');
